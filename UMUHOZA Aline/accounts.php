...
<section>
    ...
    <form method="post" onsubmit="return confirmInsert();">
        ...
        <input type="text" id="ctype" name="ctype" required><br><br>

        <label for=bal>Balance:</label>
        <input type="number" id="bal" name="bal" required><br><br>

        <label for="opdate">OpenDate:</label>
        <input type="date" id="opdate" name="opdate" required><br><br>
...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    $stmt = $connection->prepare("INSERT INTO Accounts(AccountID, CustomerID, AccountType, Balance,OpenDate) VALUES (?, ?, ?, ?, ?)");
    $stmt->bind_param("sssss", $aid, $cid, $ctype, $bal, $opdate);
...
<head>
    <meta charset="utf-8">
    <meta name="viewport" content="width=device-width, initial-scale=1">
    <title>Detail information Of Accounts</title>
    <style>
        table {
            width: 100%;
...
<body>
    ...
    <table border="5">
       ...
       include('database_connection.php');
        ...
        // Prepare SQL query to retrieve all Accounts
        $sql = "SELECT * FROM accounts";
        $result = $connection->query($sql);
        ...
        if ($result->num_rows > 0) {
            ...
            while ($row = $result->fetch_assoc()) {
                $aid = $row['AccountID']; // Fetch the AccountID
                echo "<tr>
                    <td>" . $row['AccountID'] . "</td>
                    ...
                    <td>" . $row['Balance'] . "</td>
                    <td>" . $row['OpenDate'] . "</td>
                    <td><a style='padding:4px' href='delete_accounts.php?AccountID=$aid'>Delete</a></td> 
                    <td><a style='padding:4px' href='update_accounts.php?AccountID=$aid'>Update</a></td> 
                </tr>";
            }
        } else {
