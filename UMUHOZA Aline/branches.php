...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    $stmt = $connection->prepare("INSERT INTO branches(BranchID, BranchName, Location) VALUES (?, ?, ?)");
    $stmt->bind_param("iss", $bid, $bname, $loc);
...
// SQL query to fetch data from the branches table
...
<head>
    <meta charset="utf-8">
    <meta name="viewport" content="width=device-width, initial-scale=1">
    <title>Detail information Of Branches</title>
    <style>
        table {
            width: 100%;
...
<body>
    ...
    <table border="5">
      ...
      include('database_connection.php');
        ...
        // Prepare SQL query to retrieve all Branches
        $sql = "SELECT * FROM branches";
        $result = $connection->query($sql);
        ...
        if ($result->num_rows > 0) {
            ...
            while ($row = $result->fetch_assoc()) {
                $bid = $row['BranchID']; // Fetch the BranchID
                echo "<tr>
                    <td>" . $row['BranchID'] . "</td>
                    <td>" . $row['BranchName'] . "</td>
                    <td>" . $row['Location'] . "</td>
                    <td><a style='padding:4px' href='delete_branches.php?BranchID=$bid'>Delete</a></td> 
                    <td><a style='padding:4px' href='update_branches.php?BranchID=$bid'>Update</a></td> 
                </tr>";
            }
        } else {
